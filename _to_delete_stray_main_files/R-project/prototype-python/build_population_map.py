 ...
 const rec=D.sc[iso]||[];
 {
  ...
  h+=`<div class="psec"><h3>Drafting brief — what the evidence says about the questionnaire here</h3><div class="brief">`+
   `<div class="bk"><h4>1 · Who is here → who to identify</h4>`+
    ...
    (previewPop?`; preview it for <b>${previewPop}</b>, which swaps in that population's own forced-to-flee examples, lists ${previewPop} as its own answer at FleeLoc, and fills LocLiv and CitLoc with ${previewPop} instead of ${d.name} \u2014 citizenship is asked about the country the home was fled in, not the country of interview`:"")+`.</div></div>`+
