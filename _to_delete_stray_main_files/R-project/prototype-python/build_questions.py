...
def main():
    ...
    n_cust = 0
    for iso, v in out.items():
        # The short form of each hosted population's name. It is what goes into
        # the questionnaire when that population is previewed - FleeLoc's
        # pre-listed answer and the LocLiv/CitLoc fill - because "Venezuela" is
        # what an interviewer reads out, not "Venezuela (Bolivarian Republic of)".
        for pp in (v.get("populations") or []):
            pp["short"] = clean_country(pp["name"]) if pp.get("kind") == "refugee" else pp["name"]
        cust = {}
...
function buildPops(v){
 ...
 wrap.querySelectorAll('.pop').forEach(b=>b.addEventListener('click',()=>{
  ...
  // The whole module moves with the population, not just the examples: FleeLoc
  // gains the origin as its own coded answer and the LocLiv/CitLoc fills resolve
  // to it, so the chain has to be rebuilt too.
  buildPops(v); render(); renderReg(sel.value);}));}
...
function optRows(opts,arrows,codes){
 return `<div class="modq-opts">`+opts.map((o,i)=>
  `<div class="modq-opt"><span class="box"></span>${codes&&codes[i]!=null?codes[i]:i+1}. ${o}`+
...
// The country the causing event happened in. FleeLoc establishes it, and every
// later item that names a country must name THAT country, not the survey
// country: someone interviewed in Peru who fled a home in Venezuela was never
// a citizen of Peru, and asking "had you always lived in Peru?" mis-frames the
// whole sequence. The fielded FDS resolves this as FleeLoc_label / cit_loc_c;
// here it renders as a fill token, resolved to the previewed population's
// origin when one is being previewed (step 3, More options).
function fledName(){
 try{ const {pop,usingPop}=formData(); return (usingPop&&pop&&pop.kind!=="national")?(pop.short||pop.name):null; }
 catch(e){ return null; }
}
const fledFill=()=>{ const n=fledName();
 // Brackets are part of the text, not CSS, so they survive every download.
 return `<span class="fill${n?" filled":""}">[${esc(n||MT().ui.fill_fled)}]</span>`; };
const fillF=str=>str.split("{country}").join(fledFill());
...
function itemHTML(key,ctx){
 ...
 switch(key){
  ...
  case "fleeloc": {
   const fl=fledName();
   // Codes are the module's, not the row's: the survey country stays 1 and
   // "other country [SPECIFY]" stays 2 whatever is listed between them, so a
   // pre-listed origin is coded 10+ exactly as the export writes it.
   const opts=[t.fleeloc.opts[0]+" &mdash; "+EG(ctx.c)], codes=[1], arr=[null];
   if(fl){ opts.push(EG(fl)); codes.push(10); arr.push(null); }
   opts.push(t.fleeloc.opts[1]); codes.push(2); arr.push(fwMode()==="idp"?u.oos_idp:null);
   return modq("FleeLoc",t.fleeloc.skip,t.fleeloc.stem,
    optRows(opts,arr,codes)+
    (fl?`<div class="modq-note">${u.pop_fill.split("{p}").join(esc(fl))}</div>`:"")+
    // an origin already listed as its own answer is not also an "e.g."
    ((rest=>rest.length?custLine(u.cust_other,rest,"origins"):"")((cu.origins||[]).filter(o=>o!==fl))));
  }
  ...
  case "locliv": {
   const note=`<div class="modq-note">${fillC(u.note_fled,ctx)}</div>`;
   return modq("LocLiv",t.locliv.skip,fillF(t.locliv.stem),yesno(t)+note)+
          modq("CitLoc",t.locliv.skip,fillF(t.citloc.stem),yesno(t)+note);
  }
...
.eg{color:#3b71b9}
.fill{color:#3b71b9;font-weight:700;font-style:normal}
...
function customisationHTML(iso){
 ...
 if(pop) h+=row("Population previewed",esc(pop.name)+` &mdash; the forced-to-flee examples shown are `+
   ...
   (pop.kind!=="national"?`. ${esc(pop.name)} is also offered as its own coded answer at FleeLoc (code 10), and the LocLiv and CitLoc fills read ${esc(pop.name)} in this preview`:""));
 if(itemActive("locliv")) h+=row("LocLiv and CitLoc &mdash; which country is named",
   `Neither names ${esc(v.name)}. Both read out <b>[the country named at FleeLoc]</b>: the survey country when FleeLoc = 1, otherwise the country the respondent fled a home in. `+
   `The export carries this as the calculated field <code>FleeLoc_label</code>, the same construction the FDS fields. `+
   `Citizenship at the time of the causing event is what the frameworks condition on, so asking it about the survey country would mis-classify anyone who fled a home abroad.`);
...
const INSTR={
 ...
 locliv:{name:"LocLiv / CitLoc", title:"Always lived there / citizenship",
  ...
  how:"Both questions carry a TEXT FILL, shown in the questionnaire as [the country you named at FleeLoc]. Read the country the respondent named at FleeLoc, not the country you are interviewing in. &ldquo;Always lived&rdquo; ignores short or temporary absences - holidays, seasonal work, study.",
  defs:["<b>Always lived</b>: their usual residence was there; short absences do not break it.",
        "<b>The country in the fill</b>: the survey country when FleeLoc = 1, otherwise the country the respondent fled a home in. On paper, write that country into the blank before you read the question; on tablet, the software fills it (<code>FleeLoc_label</code>)."],
  ...
  errors:["Reading the survey country instead of the country fled from - the single most damaging error in this pair, because citizenship AT THE PLACE OF THE CAUSING EVENT is the framework condition. Someone interviewed in the survey country who fled a home abroad was very likely never a citizen here, and a Yes recorded against the wrong country mis-classifies them.",
          "Treating a season of work abroad as breaking &ldquo;always lived&rdquo;."]},
...
function instrCountryBox(key,iso,v,r,mode){
 ...
 if(key==="fleeloc"&&cu.origins&&cu.origins.length){
  bits.push(`<b>Likely &ldquo;other country&rdquo; answers here:</b> ${cu.origins.map(esc).join(", ")} &mdash; the largest displaced populations hosted in this country (UNHCR). `+
   `Pre-listing an origin as its own coded answer (code 10 and up) saves a write-in and makes the LocLiv and CitLoc fill exact; the export does this for the origins listed above.`);
 }
 if(key==="locliv"){
  const fl=fledName();
  bits.push(`<b>Which country goes in the blank.</b> Not ${esc((Q[iso]||{}).name||iso)} by default. Read the country recorded at FleeLoc: `+
   `the survey country if FleeLoc = 1, the pre-listed origin if one was coded, otherwise the country written in at FleeLoc.`+
   (fl?` In this preview the fill reads <b>${esc(fl)}</b>.`:""));
 }
...
const VARS=[
 ...
 ["fleeloc","FleeLoc","1 survey country · 2 other country (fleeloc_oth holds the name) · 10+ a pre-listed origin country, one code each. Anything other than 1 means the home fled was abroad."],
 ["fleeloc_label","derived","the name of the country the home fled was in — the survey country when fleeloc = 1, the pre-listed origin when fleeloc ≥ 10, otherwise fleeloc_oth. Read out inside LocLiv and CitLoc."],
 ["locliv","LocLiv","1 yes · 2 no — always lived in fleeloc_label"],
 ["citloc","CitLoc","1 yes · 2 no — citizen of fleeloc_label when the home there was fled (asked of everyone who fled)"],
...
function ruleSet(){
 ...
 if(mode!=="idp"){
  ...
  const OUT=FD.concat([[nin("fleeloc",[1])]]);
  if(has.legal){
 ...
 if(mode==="idp") add(29,"Displaced from a home in another country - out of scope for the IDP questionnaire","ref",FD.concat([[nin("fleeloc",[1])]]),null,"—");
...
function condPlain(cond){
 ...
 const at=a=>{ const n=NM[a[0]]||a[0], vv=x=>(VAL[a[0]]&&VAL[a[0]][x]!=null)?VAL[a[0]][x]:x;
  if(a[0]==="fleeloc"&&a[1]==="nin"&&a[2].length===1&&a[2][0]===1) return "FleeLoc = a country other than the survey country";
...
function walkPath(a){
 ...
 asked.push("FleeLoc");
 if(mode==="idp"&&a.fleeloc!=null&&a.fleeloc!==1) { if(A("legal")) asked.push("Legal"); return {asked,valid,end:"out of scope"}; }
 ...
 if((a.fleeloc!=null&&a.fleeloc!==1)||(askCross&&a.fleecross===1)){
  asked.push("Apply");
  if(mode!=="idp"){
...
function walkRender(){
 ...
 const fl=fledName();
 grid.innerHTML=WK_FIELDS.filter(([k])=>inQ[k]).map(([k,lab,opts0])=>{
  // A previewed origin is a real FleeLoc code (10+), so it is walkable too.
  const opts=(k==="fleeloc"&&fl)?[opts0[0],[10,fl],opts0[1]]:opts0;
...
function translationRows(iso){
 ...
 if(A("locliv")){
  // These two carry a text fill, not a country name: what a translator must
  // keep is the placeholder, so it is shown as one rather than resolved.
  const FLED="[COUNTRY FLED FROM]";
  add("LocLiv","Skip rule",per(l=>mt(l).locliv.skip),"Not read aloud.");
  add("LocLiv","Question",per(l=>mt(l).locliv.stem.split("{country}").join(FLED)),
      "KEEP [COUNTRY FLED FROM] as a placeholder - the software fills it from FleeLoc. Do not write the survey country here.");
  add("CitLoc","Skip rule",per(l=>mt(l).citloc.skip),"Not read aloud.");
  add("CitLoc","Question",per(l=>mt(l).citloc.stem.split("{country}").join(FLED)),
      "KEEP [COUNTRY FLED FROM] as a placeholder - the software fills it from FleeLoc. Do not write the survey country here.");
  add("LocLiv / CitLoc","Interviewer note",per(l=>u(l).note_fled.split("{country}").join(v.name)),"Not read aloud.");
 }
...
function xlsFormRows(iso){
 ...
 // The country the causing event happened in, resolved exactly as the fielded
 // FDS form resolves FleeLoc_label / cit_loc_c. LocLiv and CitLoc read this
 // out; naming the survey country there is wrong for anyone who fled a home
 // abroad, which is most of a refugee sample.
 const qs=x=>String(x).split("'").join("");
 const flLab=(cu.origins||[]).reduce((acc,o,i)=>`if(\${FleeLoc}=${10+i}, '${qs(o)}', ${acc})`,
   `if(\${FleeLoc}=2, \${FleeLoc_specify}, '${qs(C)}')`);
 rows.push({type:"calculate",name:"FleeLoc_label",calculation:flLab});
 ...
 // ---- LocLiv / CitLoc ----
 if(A("locliv")){
  const FLED="${FleeLoc_label}";
  row("select_one ynr","LocLiv",perLang(m=>m.locliv.stem.split("{country}").join(FLED)),{relevant:FV,required:"yes"});
  row("select_one ynr","CitLoc",perLang(m=>m.citloc.stem.split("{country}").join(FLED)),{relevant:FV,required:"yes"});
 }
 // ---- FleeCross ----
