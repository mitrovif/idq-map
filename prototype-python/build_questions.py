...
const NA_LABEL={roster:"roster",extra:"more questions",composite:"composite measure"};
...
function buildModPicker(){
 ...
 const card=(key,title,on,cats,coreText)=>{
  ...
  let body="";
  if(key&&!on) body=`<p class="modoff">Not being identified. Tick to add its categories.</p>`;
  else{
   body=coreText||"";
   let lastGrp=null;
...
function wireDownload(btnId,builder,suffix,asPdf){
 document.getElementById(btnId).addEventListener('click',async()=>{
  ...
  }catch(e){ setDlStatus(asPdf?'Could not build the PDF.':'Could not build the Word document.',true); }
 });}
